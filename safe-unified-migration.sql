-- =====================================================
-- Safe Unified Model Management System Migration
-- =====================================================
-- This version checks for existing structures before creating

-- Step 1: Create the unified models table (if not exists)
-- =====================================================
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_models_active') THEN
        CREATE INDEX idx_models_active ON models(is_active, model_type);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_models_featured') THEN
        CREATE INDEX idx_models_featured ON models(is_featured);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_models_tags') THEN
        CREATE INDEX idx_models_tags ON models USING GIN(tags);
    END IF;
END $$;

-- Step 2: Create model sources table
-- =====================================================
CREATE TABLE IF NOT EXISTS model_sources (
    ...
    -- Add foreign key only if models table exists and has data
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_model_sources_routing') THEN
        CREATE INDEX idx_model_sources_routing ON model_sources(model_id, is_available, priority DESC);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_model_sources_provider') THEN
        CREATE INDEX idx_model_sources_provider ON model_sources(provider_name, is_available);
    END IF;
END $$;

-- Step 3: Create routing logs table
-- =====================================================
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_routing_logs_model') THEN
        CREATE INDEX idx_routing_logs_model ON model_routing_logs(model_id, created_at);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_routing_logs_provider') THEN
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_tables 
        WHERE tablename = 'model_routing_logs' 
        AND rowsecurity = true
    ) THEN
