...
SELECT 
    ...
    mc.context_window,
    mc.max_tokens,
    NULL, -- custom_name 不存在
    mc.input_price,
    mc.output_price,
    mc.tier_required::text,
    CASE 
    ...
    COALESCE(mc.health_status, 'healthy'),
    mc.health_message
...
SELECT 
    ...
    'direct',
    mc.provider,
    COALESCE(mc.provider_model_id, mc.model),
    mc.input_price,
    mc.output_price,
    CASE 
...
-- 首先获取聚合器名称（从 provider_id 关联到 api_providers 表）
WITH aggregator_info AS (
    SELECT 
        am.*,
        ap.name as aggregator_name,
        ap.base_url as api_endpoint,
        COALESCE((am.pricing->>'input_cost')::numeric, 0) as input_price,
        COALESCE((am.pricing->>'output_cost')::numeric, 0) as output_price
    FROM aggregator_models am
    LEFT JOIN api_providers ap ON ap.id = am.provider_id
)
-- 插入或更新 models 表
INSERT INTO models (
    model_id,
...
SELECT DISTINCT
    ai.model_id,
    ai.display_name,
    'Model from ' || COALESCE(ai.aggregator_name, 'aggregator'),
    ai.model_type,
    ai.capabilities,
    ai.context_window,
    ...
    'free',
    CASE 
        WHEN ai.model_name LIKE '%gpt-4%' THEN ARRAY['powerful']
        WHEN ai.model_name LIKE '%claude%' THEN ARRAY['powerful']
        WHEN ai.model_name LIKE '%gpt-3.5%' THEN ARRAY['fast']
        ELSE ARRAY[]::TEXT[]
    END,
    COALESCE(ai.is_available, true),
    COALESCE(ai.is_featured, false),
    ai.training_cutoff
FROM aggregator_info ai
ON CONFLICT (model_id) DO UPDATE SET
    ...
    END,
    -- Update context window if aggregator has larger
    context_window = GREATEST(EXCLUDED.context_window, models.context_window);
...
WITH aggregator_info AS (
    SELECT 
        ...
        ap.name as aggregator_name,
        ap.base_url as api_endpoint,
        COALESCE((am.pricing->>'input_cost')::numeric, 0) as original_input_price,
        COALESCE((am.pricing->>'output_cost')::numeric, 0) as original_output_price
    FROM aggregator_models am
    LEFT JOIN api_providers ap ON ap.id = am.provider_id
)
INSERT INTO model_sources (
...
SELECT 
    ai.model_id,
    'aggregator',
    COALESCE(ai.aggregator_name, 'unknown'),
    ai.model_id, -- aggregator 可能使用相同的 model_id
    ai.original_input_price,
    ai.original_output_price,
    CASE 
        WHEN ai.aggregator_name = 'openrouter' THEN 90
        WHEN ai.aggregator_name = 'aigateway' THEN 85
        ELSE 80
    END,
    100,
    ai.api_endpoint,
...
WHERE model_id LIKE '%turbo%' OR model_id LIKE '%flash%' OR model_id LIKE '%haiku%' OR model_id LIKE '%mini%';
